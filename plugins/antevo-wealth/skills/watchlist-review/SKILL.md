...
description: >-
  ...
  connector's wealth.watchlist tool (price, momentum, how its indicators lean, volatility per
...
## Step 1 — Gather
- `list_watchlist(household_id)` — the tracked instruments with price, 1D/1W/1M/YTD moves, `signal_lean` (up / down / neutral) with its `signal_score`, regime, and volatility chips, grouped by asset type.

## Step 2 — Analyse
- **Movers.** Biggest 1D / YTD moves.
- **Lean.** Which names lean up or down, and anything that changed side. Say "leans down", never "a sell" — it is an indicator reading, not a call.
...
**Worth attention.** {1–2 lines: the names that moved or changed lean.}

## By asset type
- {ticker}: {price} · {1D/YTD} · leans {up/down/neutral} · vol {…}
